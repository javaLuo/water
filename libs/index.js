...
function initAllTexturesAndImgs() {
    ...
    loader.load("./libs/imgs/lightray_red.jpg", function(texture){
    ...
    loader.load("./libs/imgs/001_electric.jpg", function(texture){
...
function animate_rayhover(){
    ...
    if(raycaster.intersectObjects([water_mesh]).length){ // 被选中
        ...
        if(hoverToDo) {
            ...
            for(let i=0; i<cone_group.children.length;i+=2) {
                if(i === now && cone_group.children[now].scale.x < 1) {
                    ...
                    cone_group.children[i].scale.set(scale, 1, scale);
                    cone_group.children[i+1].scale.set(scale, 1, scale);
                    cone_group1.children[i].scale.set(scale, 1, scale);
                    cone_group1.children[i+1].scale.set(scale, 1, scale);
                    cone_group2.children[i].scale.set(scale, 1, scale);
                    cone_group2.children[i+1].scale.set(scale, 1, scale);
                }
            }
        }
    } else { // 未被选中
        ...
        if(noHoverToDo) {
            ...
            for(let i=0; i<cone_group.children.length;i++) {
                cone_group.children[i].scale.set(0.01, 0.01, 0.01);
                cone_group1.children[i].scale.set(0.01, 0.01, 0.01);
                cone_group2.children[i].scale.set(0.01, 0.01, 0.01);
            }
        }
    }
}
